...
@Controller
@RequestMapping(value = "Chur")
public class ChurController {
	@Autowired
	private ChurMapper churMapper;
	@Autowired
	...
	public String initUtil(HttpServletRequest request, Model model) {
		...
		return "Chur/saveOrUpdate";
	}

	@RequestMapping(value = "/selectUtil.do")
	public String selectUtil(HttpServletRequest request, Chur util, Model model) {
		util = churMapper.selectObject(util.getId());
	...
	public String getAllUtil(HttpServletRequest request, Model model) {
		...
		List<Chur> list = churMapper.getObjectList(field, fieldValue);
		List<Chur> listChur = new ArrayList<Chur>();
	...
	public String deleteUtil(HttpServletRequest request, Chur util, Model model) {
		try{
			churMapper.deleteObject(util.getId());
		}catch(Exception e){
		}
	...
	public String saveOrUpdateObject(HttpServletRequest request, Chur util, Model model) {
		//List<Chur> list = churMapper.getObjectList("s_0", util.getS_0());
		List<User> listUser = userMapper.getObjectList(null, null);
		model.addAttribute("listUser", listUser);
		if (0 == util.getId()) {
			//if (list.size() > 0) {

...
public class TouSuController {
	...
	public String initUtil(HttpServletRequest request, Model model) {
		...
		return "TouSu/saveOrUpdate";
	}

	@RequestMapping(value = "/selectUtil.do")
	public String selectUtil(HttpServletRequest request, TouSu util, Model model) {
		util = touSuMapper.selectObject(util.getId());
	...
	public String getAllUtil(HttpServletRequest request, Model model) {
		...
		List<TouSu> list = touSuMapper.getObjectList(field, fieldValue);
		List<TouSu> listTouSu = new ArrayList<TouSu>();
	...
	public String deleteUtil(HttpServletRequest request, TouSu util, Model model) {
		try{
			touSuMapper.deleteObject(util.getId());
		}catch(Exception e){
		}
	...
	public String saveOrUpdateObject(HttpServletRequest request, TouSu util, Model model) {
		//List<TouSu> list = touSuMapper.getObjectList("s_0", util.getS_0());
		List<User> listUser = userMapper.getObjectList(null, null);
		model.addAttribute("listUser", listUser);
		if (0 == util.getId()) {
			//if (list.size() > 0) {

...
public class BaoXiuController {
	...
	public String selectUtil(HttpServletRequest request, BaoXiu util, Model model) {
		util = baoXiuMapper.selectObject(util.getId());
	...
	public String getAllUtil(HttpServletRequest request, Model model) {
		...
		List<BaoXiu> listBaoXiu = new ArrayList<BaoXiu>();
		HttpSession session = request.getSession();
		int user_id = (Integer) session.getAttribute("user_id");
		String user_type = (String) session.getAttribute("user_type");
		for (BaoXiu temp : list) {
	...
	public String deleteUtil(HttpServletRequest request, BaoXiu util, Model model) {
		try{
			baoXiuMapper.deleteObject(util.getId());
		}catch(Exception e){
		}
	...
	public String saveOrUpdateObject(HttpServletRequest request, BaoXiu util, Model model) {
		//List<BaoXiu> list = baoXiuMapper.getObjectList("s_0", util.getS_0());
		List<User> listUser = userMapper.getObjectList(null, null);
		model.addAttribute("listUser", listUser);
		if (0 == util.getId()) {
			//if (list.size() > 0) {
			...
			//	return "BaoXiu/saveOrUpdate";
			//}
			baoXiuMapper.insertObject(util);
		} else {
			//if (list.size() > 1) {
			//	model.addAttribute("util", util);
			//	model.addAttribute("errMsg", "该信息已存在!");
			//	return "BaoXiu/saveOrUpdate";
			//}
			baoXiuMapper.updateObject(util);
		}
		return this.getAllUtil(request, model);
	}
}

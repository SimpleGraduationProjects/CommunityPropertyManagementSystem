		...
		<div id="Member_Ratings">
			<div class="d_Confirm_Order_style">
				...
				<%-- <c:if test="${user_type == 'user'}"><div class="border clearfix">
					<span class="l_f"> <a
						href="${pageContext.request.contextPath}/ZhuFang/initUtil.do"
						class="btn btn-warning"><i class="icon-plus"></i>添加住房</a> </span>
				</div></c:if> --%>
				<c:if test="${user_type == 'admin'}"><div class="border clearfix">
					<span class="l_f"> <a
						href="${pageContext.request.contextPath}/ZhuFang/initUtil.do"
						class="btn btn-warning"><i class="icon-plus"></i>添加住房</a> <a
				...
				<div class="table_menu_list">
					<form
						action="${pageContext.request.contextPath}/ZhuFang/deleteManyUtil.do"
						...
						<table class="table table-striped table-bordered table-hover"
							...
							<thead>
								<tr>
									...
									<th width="100">迁入时间</th>
									<th width="100">住房类型</th>
									<th width="100">楼宇单元</th>
									<th width="100">房号</th>
									<th width="100">房型</th>
									<th width="100">朝向</th>
									<th width="100">建筑面积</th>
									<th width="100">住房介绍</th>
									<th width="100">操作</th>
								</tr>
							</thead>
							<tbody>
								...
								<c:forEach var="tmp" items="${pageModel.list}">
									<tr>
										...
										<td class="td-manage"><c:if test="${user_type == 'admin'}"><a title="编辑" class="btn btn-xs btn-info"
											href="${pageContext.request.contextPath}/ZhuFang/selectUtil.do?id=${tmp.id }"><i
												class="icon-edit bigger-120"></i> </a> <a title="删除"
											href="javascript:;"
											onclick="member_del('${pageContext.request.contextPath}/ZhuFang/deleteUtil.do?id=${tmp.id }')"
...
<script type="text/javascript">
	jQuery(function($) {
		 $('#sample-table').dataTable( {
			...
			"aoColumnDefs": [
				  {"orderable":false,"aTargets":[0,9]}// 指定列不参与排序
			] } );
	});
</script>

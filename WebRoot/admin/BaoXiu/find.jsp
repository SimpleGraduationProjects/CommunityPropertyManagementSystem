		...
		<div id="Member_Ratings">
			<div class="d_Confirm_Order_style">
				...
				<c:if test="${user_type == 'user'}"><div class="border clearfix">
					<span class="l_f"> <a
						href="${pageContext.request.contextPath}/BaoXiu/initUtil.do"
						class="btn btn-warning"><i class="icon-plus"></i>添加报修</a> </span>
				</div></c:if>
				<c:if test="${user_type == 'admin'}"><div class="border clearfix">
					<span class="l_f"> <a
						href="${pageContext.request.contextPath}/BaoXiu/initUtil.do"
						class="btn btn-warning"><i class="icon-plus"></i>添加报修</a> <a
				...
				<div class="table_menu_list">
					<form
						action="${pageContext.request.contextPath}/BaoXiu/deleteManyUtil.do"
						...
						<table class="table table-striped table-bordered table-hover"
							...
							<thead>
								<tr>
									...
									<th width="100">报修人</th>
									<th width="100">报修名称</th>
									<th width="100">报修日期</th>
									<th width="100">报修类型</th>
									<th width="100">报修内容</th>
									<th width="100">报修说明</th>
									<th width="100">报修状态</th>
									<th width="100">操作</th>
								</tr>
							</thead>
							<tbody>
								...
								<c:forEach var="tmp" items="${pageModel.list}">
									<tr>
										...
										<td class="td-manage"><c:if test="${user_type == 'admin'}"><a title="编辑" class="btn btn-xs btn-info"
											href="${pageContext.request.contextPath}/BaoXiu/selectUtil.do?id=${tmp.id }"><i
												class="icon-edit bigger-120"></i> </a> <a title="删除"
											href="javascript:;"
											onclick="member_del('${pageContext.request.contextPath}/BaoXiu/deleteUtil.do?id=${tmp.id }')"
...
<script type="text/javascript">
	jQuery(function($) {
		 $('#sample-table').dataTable( {
			...
			"aoColumnDefs": [
				  {"orderable":false,"aTargets":[0,6]}// 指定列不参与排序
			] } );
	});
</script>

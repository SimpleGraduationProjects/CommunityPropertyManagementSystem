		...
		<div id="Member_Ratings">
			<div class="d_Confirm_Order_style">
				...
				<c:if test="${user_type == 'user'}"><div class="border clearfix">
					<span class="l_f"> <a
						href="${pageContext.request.contextPath}/PingJia/initUtil.do"
						class="btn btn-warning"><i class="icon-plus"></i>添加留言建议</a> </span>
				</div></c:if>
				<c:if test="${user_type == 'admin'}"><div class="border clearfix">
					<span class="l_f"> <a
						href="${pageContext.request.contextPath}/PingJia/initUtil.do"
						class="btn btn-warning"><i class="icon-plus"></i>添加留言建议</a> <a
				...
				<div class="table_menu_list">
					<form
						action="${pageContext.request.contextPath}/PingJia/deleteManyUtil.do"
						...
						<table class="table table-striped table-bordered table-hover"
							...
							<thead>
								<tr>
									...
									<th width="100">留言人</th>
									<th width="100">留言名称</th>
									<th width="100">留言日期</th>
								...
								<c:forEach var="tmp" items="${pageModel.list}">
									<tr>
										...
										<td class="td-manage"><c:if test="${user_type == 'admin'}"><a title="编辑" class="btn btn-xs btn-info"
											href="${pageContext.request.contextPath}/PingJia/selectUtil.do?id=${tmp.id }"><i
												class="icon-edit bigger-120"></i> </a> <a title="删除"
											href="javascript:;"
											onclick="member_del('${pageContext.request.contextPath}/PingJia/deleteUtil.do?id=${tmp.id }')"
...
<script type="text/javascript">
	jQuery(function($) {
		 $('#sample-table').dataTable( {
			...
			"aoColumnDefs": [
				  {"orderable":false,"aTargets":[0,4]}// 指定列不参与排序
			] } );
	});
</script>
